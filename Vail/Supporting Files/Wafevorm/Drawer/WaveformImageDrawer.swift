...
class WaveformImageDrawer: ObservableObject {
    ...
    func waveformImage(fromAudioAt audioAssetURL: URL,
...
extension WaveformImageDrawer {
    ...
    func draw(waveform samples: [Float], on context: CGContext, with configuration: Waveform.Configuration, renderer: WaveformRenderer) {
...
extension WaveformImageDrawer {
    ...
    ///
    /// Samples need to be normalized within interval `(0...1)`.
    func waveformImage(from samples: [Float], with configuration: Waveform.Configuration, renderer: WaveformRenderer) -> UIImage? {
